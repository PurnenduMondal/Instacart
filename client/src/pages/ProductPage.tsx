...
import { ArrowLeftIcon, Home, Leaf, LeafIcon, MinusIcon, PlusIcon, ShoppingCartIcon, StarIcon } from 'lucide-react'

const ProductPage = () => {

  const corrency = import.meta.env.VITE_CURRENCY_SYMBOL || "$"
  const { id } = useParams()
  ...
  return (
    <div className='min-h-screen'>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 1g:px-8 py-6">
        {/* Breadcrumb */}
        ...
        {/* Product Details Section */}
          <div className="bg-white/50 rounded-2x1 overflow-hidden">
            <div className="grid md:grid-cols-2 gap-0">
              ...
              <div className="p-6 md:p-10 flex flex-col justify-center">
                ...
                {product.rating > 0 && (
                  <div className="flex items-center gap-2 mb-5">
                    <div className='flex items-center gap-0.5'>
                      {
                        [1,2,3,4,5].map((s) =>(
                          <StarIcon key={s} className={`w-4 h-4 ${s <= Math.round(product.rating) ? 
                ...
                <div className='flex items-baseline gap-3 mb-5'>
                  <span className='text-3xl md:text-4xl font-semibold text-app-green'>{corrency}{product.price.toFixed(2)}</span>
                  {product.originalPrice > product.price && (
                    <span className='text-lg text-app-text-light line-through'>{corrency}{product.originalPrice.toFixed(2)}</span>
                  )}
                </div>
