import { MinusIcon, PlusIcon, ShoppingBagIcon, XIcon,Trash2Icon, ArrowRightIcon } from 'lucide-react';
...
const CartSidebar = () => {
  ...
  return (
    <>
        {/* Overlay */}
        <div className="fixed inset-0 bg-black/40 z-50 transition-opacity" onClick={() => setIsCartOpen(!isCartOpen)}>
        </div>
        {/* Sidebar */}
        <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col animate-slide-in-right">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4"> 
                {items.length === 0 ? (
                    ...
                    items.map((item, i) => (
                        <div key={item.product._id} className="flex gap-3 bg-app-cream/60 rounded-x1 p-3">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center justify-between mt-2">
                                    <div className="flex items-center gap-1.5">
                                        <button onClick={() => updateQuantity(item.product._id, item.quantity - 1)} className="size-7 rounded-1g bg-white border border-app-border flex-center">
